	...
	<div :class="{ 'icon-loading': loading }">
		...
		<div v-if="!showSharingDetailsView" class="sharingTab__content">
			...
			<template v-if="!loading">
				...
				<AddAllEmailsButton :is-loading="loading" :file-info="fileInfo" :filtered-email-arr="filteredEmailArr" :on-submit="addEmailShare" @add-share="addShare" @force-update="forceUpdate"/>
			</template>
			<!-- add new share input -->
		...
		<div v-else>
			<SharingDetailsTab :file-info="shareDetailsData.fileInfo" :share="shareDetailsData.share"
				...
				@remove:share="removeShare" @update:share="updateEmailShares" />
		</div>
...
export default {
	...
	methods: {
		...
		addShare(share, resolve = () => { }) {
			...
			if (share.type === this.SHARE_TYPES.SHARE_TYPE_EMAIL || share.shareType === this.SHARE_TYPES.SHARE_TYPE_EMAIL) {
